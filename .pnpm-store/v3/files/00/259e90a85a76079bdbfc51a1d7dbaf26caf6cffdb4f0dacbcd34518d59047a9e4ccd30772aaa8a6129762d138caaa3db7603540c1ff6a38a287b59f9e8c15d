...
var require_flat = _chunkEBJAWNYGjs.__commonJS.call(void 0, {
...
// src/cli-main.ts
var import_flat = _chunkEBJAWNYGjs.__toESM.call(void 0, require_flat());
...
async function main(options = {}) {
  ...
  ).option("--publicDir [dir]", "Copy public directory to output directory").action(async (files, flags) => {
    const { build } = await Promise.resolve().then(() => require("./index.js"));
    Object.assign(options, {
      ...flags
    });
    if (!options.entry && files.length > 0) {
      options.entry = files.map(_chunkEBJAWNYGjs.slash);
    }
    if (flags.format) {
  ...
  cli.help();
  cli.version(_chunkVVG4DODSjs.version);
